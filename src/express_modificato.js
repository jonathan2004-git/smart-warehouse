//Implementazione login con sessione e routing protetto cioè solo utenti autenticati accedono alle pagine.
---------------------------------------------------------------------------------------------------------
...
const paginePubbliche = ['/index.html', '/password.html'];
...
// --- parametri di accesso al database MariaDB ---
...
// --- rotta per cambio password ---
app.post('/api/password', (req, res) => {
    const { username, vecchia_password, nuova_password } = req.body;

    // verifica che l'utente esista con la vecchia password
    db.query('SELECT * FROM utenti WHERE username = ? AND password = ?',
        [username, vecchia_password], (err, results) => {
            ...
            db.query('UPDATE utenti SET password = ? WHERE username = ?',
                [nuova_password, username], (err) => {
...
app.get('/api/sensor/latest', (req, res) => {
    ...
    db.query(sql, (err, results) => {
        ...
        res.json(results); // una riga per scaffale
    });
});

// --- ultime N letture e scelta dello scaffale ---
...
// --- ultime 24h per scaffale selezionato ---
...
app.get('/api/movimenti', (req, res) => {
    ...
    const sql = `
        SELECT m.id_movimento, m.data_movimento, m.tipo_movimento, m.quantita, m.distanza, p.nome, s.nome_scaffale, COALESCE(m.id_scaffale, ps.id_scaffale) AS id_scaffale, s.profondita
        FROM movimenti m
...
app.post('/api/prodotti', (req, res) => {
    ...
    const query = 'INSERT INTO prodotti (nome, descrizione, misura) VALUES (?, ?, ?)';

    db.query(query, [nome, descrizione, misura], (err, result) => {
        if (err) {
...
app.delete('/api/prodotti/:id', (req, res) => {
    ...
    db.query('DELETE FROM movimenti WHERE id_prodotto=?', [req.params.id], (err) => {
        ...
        db.query('DELETE FROM prodotti WHERE id_prodotto=?', [req.params.id], (err) => {
            if (err) return res.status(500).json({ error: err.message });
            db.query('SELECT COUNT(*) AS totale FROM prodotti', (err, rows) => {
                if (!err && rows[0].totale === 0) {
                    //azzera il contatore
                    db.query('ALTER TABLE prodotti AUTO_INCREMENT = 1', (errAlter) => {
                        if (errAlter) console.error("Errore nel reset dell'auto-incremento:", errAlter.message);
                        else console.log("Tabella vuota: contatore ID resettato a 1!");
                    });
                }
            });
...
app.delete('/api/scaffali/:id', (req, res) => {
    // elimina i movimenti collegati allo scaffale
    db.query('DELETE FROM movimenti WHERE id_scaffale=?', [req.params.id], (err) => {
        if (err) {
            console.error('Errore movimenti:', err.message);
            return res.status(500).json({ error: err.message });
        }

        // elimina le letture sensore collegate
        db.query('DELETE FROM letture_sensore WHERE id_scaffale=?', [req.params.id], (err) => {
            if (err) {
                console.error('Errore letture_sensore:', err.message);
                return res.status(500).json({ error: err.message });
            }

        // elimina l'associazione tra prodotti e scaffali
            db.query('DELETE FROM prodotti_scaffali WHERE id_scaffale=?', [req.params.id], (err) => {
                if (err) {
                    console.error('Errore prodotti_scaffali:', err.message);
                    return res.status(500).json({ error: err.message });
                }
            ...
            // elimina lo scaffale
                db.query('DELETE FROM scaffali WHERE id_scaffale=?', [req.params.id], (err) => {
                    if (err) {
                        console.error('Errore scaffali:', err.message);
                        return res.status(500).json({ error: err.message });
                        }
...
// vedi dove sono i prodotti negli scaffali
...
// aggiungi un prodotto a uno scaffale
...
// rimuovi un prodotto da uno scaffale
...
client.on('message', (topic, message) => {
    ...
    try {
        ...
        // inserire lo scaffale con una posizione di default e statoScaffale
