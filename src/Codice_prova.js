...
const client = mqtt.connect('mqtt://broker.hivemq.com'); // Un broker pubblico per test

// Quando il server si connette al "centralino" (Broker)
client.on('connect', () => {
    console.log('Connesso al broker MQTT');
    client.subscribe('magazzino/sensori'); // Si mette in ascolto sul canale
});
...
client.on('message', (topic, message) => {
    try {
        ...
        // 1. Usiamo i nomi reali delle tue colonne: id_scaffale, posizione, stato
        // Uso 'S' + id_scaffale se vuoi mantenere il formato "S01", oppure solo dati.id_scaffale
        ...
        // Inseriamo lo scaffale con una posizione di default (NULL) e stato 'verde'
        db.query(queryScaffale, [idScaffaleFormattato, 'Corsia da definire', 'verde'], (err) => {
            ...
            // 2. Salviamo la lettura usando l'ID formattato
